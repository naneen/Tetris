...
public class Piece {
	...
	*/
	...
	private int[] calSkirt(){
		int[] sk = new int[this.width];
		int x = 0;
		for( int i=0; i<this.width; i++){
			int y = 5000;
			for( int j=0; j<this.body.length; j++ ){
				if( this.body[j].x == i && this.body[j].y < y ){
					y = this.body[j].y;
				}
			}
			sk[i] = y;
	...
	private int calWidth( TPoint[] tpoint){
		int w = 0;
	...
	*/
	...
	public Piece computeNextRotation() {
		for( int i=0; i<bodyNew.length; i++ ){
			int change = bodyNew[i].x;
			bodyNew[i].x = bodyNew[i].y;
			bodyNew[i].y = change;
		}
		int minX = 0;
		int maxX = this.calWidth( this.bodyNew ) - 1;
		for( int i=0; i<bodyNew.length; i++ ){
			if( bodyNew[i].x == minX ){
				bodyNew[i].x = maxX;
			}
			else if( bodyNew[i].x == maxX ){
				bodyNew[i].x = minX;
			}
		}
		return	new Piece( this.bodyNew );
	}
